...
dwv.App = function()
{
     ...
     */
    this.getVersion = function() { return dwv.gui.versionRequest(); };
    ...
    this.reset = function()
    {
        ...
        imageData = null;
        // Tool box
        toolBox = new dwv.tool.ToolBox(this);
        dwv.tool.Draw.points = [];
    };
    ...
    this.loadURL = function(urls) 
    {
        ...
        this.reset();
        // create IO
//	alert("Inside loadURL");
        var urlIO = new dwv.io.Url();
        urlIO.onload = function(data){
            if( image ) {
                image.appendSlice( data.view.getImage() ); //alert("Inside if");
            }
            postLoadInit(data);
        };
